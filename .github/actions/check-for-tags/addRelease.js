    ...
    octokit.repos.getReleaseByTag({ owner, repo, tag }).then(function() {
      ...
      if (error.status === 404) {
        ...
        octokit.repos.listTags({ owner, repo }).then(function(tags){
          ...
          const sortedVersions = tags.data.filter(
            version => /^v\d+\.\d+\.\d+/.test(version.name)
          );
          const sortedVersionedTags = sortedVersions.map(v => v.name).sort(compareVersions);
          const tagSha = sortedVersions.find(version => version.name === tag).commit.sha;
